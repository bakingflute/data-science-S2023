...
-   You chose a correct value of `stat(x, y)` when `x, y` is *outside* the circle. Why did you choose this value?
    -   Since points outside the quarter circle aren't supposed to count toward the estimate of π, I decided to set the value of stat(x, y) to 0 in those cases. Anything outside of that area should not contribute to the final average because we are only interested in the portion of the square that overlaps with the quarter circle; it only makes sense that their contribution should be zero.
-   You chose a correct value of `stat(x, y)` when `x, y` is *inside* the circle. Why did you choose this value?
    -   However, since those are the points we do want to count, I set the value to 4 when the point is inside the quarter circle. Multiplying by 4 allows us to scale the estimate of π back up to fit the entire circle context. It is based on the percentage of points that fall inside that curved area. Therefore, every point inside contributes 4, and we may estimate π by averaging all of their contributions.
...
-   What is a range of plausible values, based on the sampling distribution you've generated?
    -   According to the sampling distribution, the range of reasonable values for π seems to be between 3.05 to 3.13. The distribution appears generally symmetric and bell-shaped, with the majority of the estimates clustering around the center, close to 3.09.
...
-   How closely do your bootstrap CI and CLT CI agree?
    -   (Your answer here) They agree pretty closely. Both ranges were centered close to the same location and were strikingly similar. This indicates to me that for this dataset and sample size, the bootstrap and CLT approaches produced consistent for the daa
...
-   What would be a *valid* way to make your CI more narrow?
    -   You could make the confidence interval more narrow by collecting more data — in this case, generating a larger sample of random points. With a bigger sample size, your estimate becomes more stable, and the margin of error gets smaller, which tightens the interval.
