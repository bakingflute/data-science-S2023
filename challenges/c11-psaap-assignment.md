...
- It has 140 rows (which I think means 140 different simulation runs)
  and 22 columns showing different measurements. I can see there are
  input settings like channel size and flow speed, and output
  measurements like temperature changes. The measurements were taken at
  specific spots along the channel (x values like 0.25, 0.5, etc.). Most
  temperature changes are in the middle range (median around 63%), but
  there are some really high ones that pull the average up to 74%. I
  think this means most cases behave normally but some get unusually
  hot.
...
```
    ...
    ##   <chr>             <dbl>
    ## 1 Cheat (avg_T)     0.232
    ## 2 Baseline (x only) 0.284
    ## 3 Nonphysical (idx) 0.399

**Observations**:

- Which model is *most accurate*? Which is *least accurate*?
  - The “cheat” model using avg_T is most accurate (RMSE = 0.232), while
    the model using only idx is least accurate (RMSE = 0.399). The
    baseline model with just x falls in between (RMSE = 0.284).
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is an Output variable (the measured average temperature). It’s
    super effective because T_norm is mathematically derived from it
    (T_norm = (avg_T - T_f)/T_f), creating a perfect circular
    relationship that wouldn’t exist with real-world prediction tasks.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No - we wouldn’t have avg_T when trying to predict new T_norm
    values. While it technically works mathematically, it’s not a valid
    predictor because: it requires knowing the answer to predict the
    answer, creates data leakage, defeats the purpose of prediction.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is a Metadata variable - just a simulation ID number. It has
    zero physical meaning regarding the fluid dynamics; it’s simply an
    arbitrary counter tracking which simulation run produced the data.
    Using it as a predictor is meaningless since simulation \#5 isn’t
    inherently different from \#4 in terms of the physics.
...
- Which columns are excluded in the model formula above? What categories
  ...
  the model?
  - The model excludes: Metadata variables (idx) - Just simulation IDs
    with no physical meaning, output measurements (avg_q, avg_t, rms_T),
    and many physical inputs.
- Which inputs are *statistically significant*, according to the model?
  - At the traditional p \< 0.05 level:
    - Definitely significant: W (width, p \< 0.0001) and T_f (inlet
      temp, p \< 0.0001)
...
- How do these standard deviations relate to the regression coefficients
  for `x` and `T_f`?
  - The larger standard deviation for T_f helps explain why its
    coefficient is smaller - a 1-unit change represents a much smaller
    relative change for T_f than for x. This shows why we can’t directly
    compare raw coefficients to judge importance.
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - This shows that statistical significance (p-values) depends heavily
    on:
    1.  Sample size - with enough data, tiny effects become
        “significant”

    2.  Measurement precision - variables with smaller variation appear
        more significant
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - The plot compares two models - the simple “x only” model and the
    more complex “q4” model. The q4 model appears more accurate because
    of its predicted values are much more clustered tightly around the
...
- Which model tends to be *more confident* in its predictions? Put
  ...
  - The q4 model shows narrower prediction intervals (tighter error
    bars) compared to the x-only model. The increased confidence comes
    from incorporating additional meaningful predictors that reduce
    uncertainty in the predictions. The simple x-only model has wider
    intervals because it’s working with less information.
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - The fit_simple (“x only”) model requires just 1 predictor (x) to
    make predictions. In contrast, the fit_q4 model needs multiple
    predictors - from the previous question we know it uses L, W, U_0,
    N_p, k_f, and T_f (6 predictors total). While the more complex model
    requires more input information, the plot shows this pays off in
    both better accuracy and greater confidence in predictions.
...
- How much do you trust your model? Why?
  - I think the mode is okay. I found that 93.3% of actual T_norm values
    fell inside your 80% Pls, so the model is capturing variability– and
    even errs on the side of being slightly conservative.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - Prediction intervals; the team wants a range that an individual
    *future* prototype is likely to exhibit, not an interval for the
    *mean* T_norm.
